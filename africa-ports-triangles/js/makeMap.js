...
function Map(container, properties) {
  ...
  this.render = function() {
    _this.map = L.map(container, {
      minZoom: _this.minzoom || null,
      maxZoom: _this.maxzoom || 20,
      maxBounds: _this.maxbounds || null,
      scrollWheelZoom: window.innerWidth < 768 ? false : true,
      zoomControl:
        !_this.hasOwnProperty("zoomslider") || _this.zoomslider ? false : true,
      attributionControl: false
    });
    if (_this.loadEvent) _this.map.on("load", _this.loadevent);
    if (_this.addEvent) _this.map.on("layeradd", _this.addevent);

    this.map.setView(_this.center, _this.zoom);
    L.tileLayer(
    ...
    if (!_this.hasOwnProperty("zoomslider") || _this.zoomslider) {
      ...
      _this.map.addControl(window.zoomSlider);
    }
...
function initWithSpreadsheet(dataURL, options, translations) {
  return fetch(
    ...
    .then(function(json) {
      ...
      var metaDataHeaders = metaData.popupheaders
        .split(",")
        .map(function(i) {
          return i.trim();
        })
      ...
      var metaDataContent = metaData.popupcontent
        .split(",")
        .map(function(i) {
          return i.trim();
        })
          ...
          (resolve, reject) => {
            Promise.all(
              ...
              .then(async function(jsons) {
                var map = await makeWidgets(jsons, properties, boxContent);
      ...
      return fetch(
        ...
        .then(function(json) {
          ...
          if (map.translations) {
            ...
            var translatableStrings = Object.keys(map.translations).sort(
              function(a, b) {
      ...
      if (column.indexOf("gsx$") > -1) {
        ...
        if (columnName === "label") {
          ...
          if (options.translations) {
            ...
            data.group = options.translations[data.group]; // debugger;
          }
