...
// CONCATENATED MODULE: ./src/india-energy.js
function india_energy_asyncGeneratorStep(gen, resolve, reject, _next, _throw, key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { Promise.resolve(value).then(_next, _throw); } }

function india_energy_asyncToGenerator(fn) { return function () { var self = this, args = arguments; return new Promise(function (resolve, reject) { var gen = fn.apply(self, args); function _next(value) { india_energy_asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value); } function _throw(err) { india_energy_asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err); } _next(undefined); }); }; }

function energy() {
  return _energy.apply(this, arguments);
}

function _energy() {
  _energy = india_energy_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
    var map;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return makeMap({
              ...
              minZoom: 5,
              clusterDistance: 15,
              Attribution: 'Data by <a href="https://www.csis.org/programs/africa-program" target="_blank">CSIS Wadhwani Chair</a>, © OpenStreetMap, Leaflet contributors, © CARTO',
              ...
              formatPopupContent: function formatPopupContent(feature, map) {
                var title = '<div class="popupTitleStyle">' + feature.properties.recipient + '</div>';
                var header = '';
                header += '<div><span class="popupHeaderStyle">Project Type: </span><span class="popupEntryStyle">' + feature.properties.project_type + '</span></div>';
                header += '<div><span class="popupHeaderStyle">Sector: </span><span class="popupEntryStyle">' + feature.properties.sector + '</span></div>';
                header += '<div><span class="popupHeaderStyle">State: </span><span class="popupEntryStyle">' + feature.properties.state + '</span></div>';
                header += '<div><span class="popupHeaderStyle">Agency: </span><span class="popupEntryStyle">' + feature.properties.agency + '</span></div>';
                header += '<div><span class="popupHeaderStyle">Year: </span><span class="popupEntryStyle">' + feature.properties.year + '</span></div>';
                var description = '<div class="popupEntryStyle"><p>' + feature.properties.description + '</p></div>';
